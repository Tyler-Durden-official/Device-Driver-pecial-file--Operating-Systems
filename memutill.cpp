		...
		switch (c) {
			...
			case 'p':
				...
				if (strcmp("demand", optarg) == 0) {
					//TODO mmap_flags = ??;
					mmap_flags = MAP_SHARED;  // Pass flag as MAP_SHARED if "demand"
				} else if (strcmp("prefetch", optarg) == 0) {
					//TODO mmap_flags = ??
					mmap_flags = MAP_SHARED | MAP_POPULATE;  // Pass flag as MAP_SHARED and MAP_POPULATE if "prefetch"
				} else {
	...
	for (int i=0; i < op_cnt; i++) {
		switch (op[i]) {
			...
			case OP_MAPREAD: {
				off_t off = 0;
				size_t len;
				dev_mem = (char *)mmap( NULL, MYDEV_LEN, PROT_READ, mmap_flags, dev_fd, off );  // memory mapping the devicemem's kernel buffer into user-space segment.
				for (int k = 0; k < MYDEV_LEN; k++) {
					if (*(dev_mem + k) != *(msg + (k % msg_len))) {	// Compare the data read from devicemem with msg.
						munmap(dev_mem, MYDEV_LEN);  // unmapping the devicemem's kernel buffer before exit failure.
						return EXIT_FAILURE; // Since data doesn't match
					}
				}
				munmap(dev_mem, MYDEV_LEN);  // unmapping the devicemem's kernel buffer.
				break;
			}
			...
			case OP_MAPWRITE: {
				off_t off = 0;
				size_t len;
				dev_mem = (char *)mmap(NULL, MYDEV_LEN, PROT_WRITE, mmap_flags, dev_fd, off);  // memory mapping the devicemem's kernel buffer into user-space segment.
				for (int k = 0; i < MYDEV_LEN; k++) {	  // Writing
					*(dev_mem + k) = *(msg + (k % msg_len));
				}
